...
export interface Config {
  ...
  signInGroupId: string[]  // 新增：允许签到的群组ID列表
}
...
export const Config: Schema<Config> = Schema.intersect([
  ...
  Schema.object({
    ...
    enableGuguBattle: Schema.boolean()  // 新增的咕咕之战开关
      .description('是否开启咕咕之战游戏功能').default(true),
  }).description('基础设置'),
  ...
  // 新增：签到配置组
  Schema.object({
    signInGroupId: Schema.array(Schema.string())
      .description('允许签到的群组ID列表（为空则不限制）').default([]),
  }).description('签到设置'),
...
export function apply(ctx: Context, config: Config) {
  ...
  // 群组权限检查工具函数
  const checkGroupAccess = (session: any, featureName: string = '该功能'): string | null => {
    // 未配置限制则不限制
...
.action(async ({ session, options }, target) => {
  ...
  if (isNumeric) {
    ...
    if (count <= validRoulettes.length) {
      // 如果指定次数小于等于轮盘数量，从前面开始抽取
      finalResults = results.slice(0, count)
    } else {
      // 如果指定次数大于轮盘数量，先取所有结果，然后随机补充
  ...
  // 批量抽奖指令
  ctx.command('roulette/批量轮盘抽奖 <ids:text>', '从多个轮盘中各抽一次奖（ID用逗号分隔）')
    .action(async ({ session }, idsText) => {
